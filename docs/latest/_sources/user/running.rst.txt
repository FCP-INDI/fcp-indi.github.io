...
If you re-run C-PAC with an output directory containing a working directory (from the runtime flag ``--save_working_dir``), C-PAC will use that working directories contents in the re-run. If you try to re-run on an output directory with a saved working directory from a different version of C-PAC than the one that you're currently running, differences in the working directory could cause problems.
