...
.. warning::
   ...
   When a developer creates or modifies a Node in C-PAC, a ``mem_gb`` argument should be provided unless the default of 0.2 is expected to be sufficient. When testing, the ``mem_gb`` argument should be adjusted if the observed memory usage of a Node exceeds the estimate.
